...
| 항목 | 내용 |
|---|---|
| 서체 | Pretendard Variable (가변 폰트, 원본 weight 축 45–930) |
...
| 축 범위 | weight **400~800** (원본 45~930 에서 좁힘). 실사용은 400·500·600·700·800 |
| 서브셋 범위 | KS X 1001 완성형 한글 **2,350자** + 라틴 기본 + 숫자 + 문장부호 + 통화기호(₩) + UI 기호(– — … ← → ○ ⚠ ✓) |
| 실측 용량 | **303KB** (한도 320KB, 여유 17KB) — 동적 서브셋 아닌 단일 파일 |
| 재생성 | `scripts/build-font.py` (uvx + fonttools). 바이너리의 출처를 재현 가능하게 남긴다 |
...
@font-face {
  ...
  unicode-range:
    U+0020-007E, U+00A0-00FF, U+20A9, U+2013-2014, U+2018-201D, U+2026,
    U+2190, U+2192, U+25CB, U+26A0, U+2713, U+3000-303F, U+AC00-D7A3,
    U+1100-11FF;
}
```

> ⚠️ **`U+AC00-D7A3` 는 2,350자가 아니라 현대 한글 11,172자 전체다.**
> 이 표기를 "완성형 2,350자"와 같은 것으로 보고 서브셋하면 파일이 **1,157KB** 가 되어
> 목표의 3.6배가 된다 (실측 2026-09-12). 파일에는 KS X 1001 2,350자만 담고,
> `unicode-range` 는 블록 전체를 선언한 채로 둔다 — 범위에 해당하는데 폰트에 없는 글자는
> 브라우저가 스택의 다음 폰트로 내려가므로 동작상 문제가 없고, 2,350개를 CSS 에
> 일일이 나열하는 것보다 훨씬 낫다. 대신 2,350자 밖의 희귀 음절(예: 힣)은
> 시스템 고딕으로 보인다 — 의도적으로 감수한 트레이드오프다.
>
> 실측 비교 (weight 400~800 가변, woff2):
>
> | 구성 | 용량 |
> |---|---|
> | 원본 가변 woff2 (서브셋 없음) | 2,009.5KB |
> | U+AC00-D7A3 전체 11,172자 | 1,157.1KB |
> | 정적 400 단일 weight · 11,172자 | 581.0KB |
> | **KS X 1001 2,350자** | **303.1KB** |
>
> `⚠️ ⓘ(U+24D8) ▾(U+25BE) ☂(U+2602)` 는 **Pretendard 원본에도 없다.** UI 가 쓰고 있지만
> 서브셋 범위와 무관하게 시스템 폰트로 폴백된다. 어떤 설정으로도 이 폰트로는 그릴 수 없다.
...
| `size-adjust` 미사용 | 가변 서브셋 단일 파일이 303KB 로 충분히 빠르다. 추가 조정은 검증 부담만 늘린다. 단 CLS 실측이 0.05 를 넘으면 이 판단을 뒤집는다 |
| 가변 폰트 1개 | 정적은 weight 당 164KB 라 400/500/600/700/800 다섯 벌이면 800KB 가 넘고 요청도 5회가 된다. CLS 관점에서도 불리하다 |
