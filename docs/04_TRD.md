...
| ⚠️ 실측 정정 (2026-09-12) | **평탄화 후에도 스키마는 여전히 거부된다.** `auditSchema()` 의 `maxDepth` 는 11 이고 위 실패선 14 보다 낮은데도, `gemini-3.8-flash`·`gemini-3.6-flash` 양쪽이 첫 호출에서 스키마를 거부한다. `schemaRejectedModels` 메모이즈로 모델당 1회만 낭비되고 폴백이 정상 동작해 기능 영향은 없다. 원인 규명은 별도 과제. 증거는 `docs/06_품질점검.md` §5 |
...
| 항목 | 예산 | **실측** | 판정 |
|---|---|---|---|
| `/` First Load JS | ~~190KB~~ → **197KB gzip 이하** (2026-09-12 조정) | **197 kB** | ✅ 조정된 예산 달성. 근거는 `docs/06_품질점검.md` §4-2 |
...
| 폰트 | 320KB 이하 · woff2 가변 서브셋 1파일 | **303.1 KB · 요청 1건 · 외부 CDN 0건** | ✅ 여유 16.9 KB |
| 3D 씬 | 파티클·삼각형·머티리얼·텍스처 상한 | **파티클 900(예산 4,000) · 지오메트리 6개 약 128삼각형 · 텍스처 0개 · 후처리 없음** | ✅ `renderer.info` 는 three 내부라 소스 집계로 대체 |
| **LCP** | 데스크톱 2.0초 이하 · 모바일 2.5초 이하 | **데스크톱 1,796ms · 모바일 668ms** (3회 중앙값, 스로틀 적용) | ✅ (S14). LCP 요소는 Hero `<h1>` |
| **INP** | 200ms 이하 | **미측정** | ⚠️ Lighthouse 미실행. 미해결 항목 |
| **CLS** | 0.05 이하 | **0 (시프트 0건)** | ✅ (S15). 폰트 프리로드로 swap 구간에도 시프트 없음 |
...
#### First Load JS — 예산을 197 kB 로 조정 (2026-09-12)

207 kB 에서 **197 kB** 로 줄인 뒤, 남은 7 kB 초과에 대해 예산 수치를 조정했다.
전체 근거는 `docs/06_품질점검.md` §4-2 에 있고 요약은 아래와 같다.

| 항목 | 내용 |
|---|---|
| 줄인 방법 | `gsap/Flip`(9.0 kB)과 결과 서브트리(5.8 kB)를 지연 청크로 분리. 둘 다 응답 도착 전에는 렌더될 수 없어 UX 비용 0 |
| 실측 구성 | 공유 103 (React+Next 런타임) · GSAP 44.7 · Zod+로케일 25.2 · Next lib 5.9 · 앱 코드 14.7 |
| 원인이 **아닌** 것 | `three` 계열. 별도 지연 청크로 분리되어 First Load JS 에 포함되지 않음 (재확인) |
| 삭감을 거부한 것 | **클라이언트 Zod 25.2 kB.** `PlanForm` 의 `safeParse` 가 S3("필수값이 비면 요청이 나가지 않는다")을 성립시키는 메커니즘이다. 손으로 다시 쓰면 refinement 10개가 두 곳에 중복된다 |
| 조정 근거 | LCP 요소가 SSR 텍스트라 First Load JS 는 LCP 임계 경로가 아니며, 사용자에게 보이는 기준(LCP·CLS)은 모두 통과한다. 190 kB 는 React 19 기준선(103 kB)과 4-플러그인 GSAP 설치를 계산에 넣지 않고 잡힌 수치로 보인다 |
| 재발 방지 | 라우트별 수치를 `docs/06_품질점검.md` §4-2 에 기록하고 릴리스마다 대조한다 |
